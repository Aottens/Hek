...
must_haves:
  truths:
    - "Safety sensors reject glitches shorter than debounce time"
    - "PBV sensor ignores pulses shorter than 50ms"
    - "VS sensor ignores pulses shorter than 100ms"
    - "FAULT condition requires both endstops active for 50ms before triggering"
    - "Timer state persists between PLC scan cycles"
  artifacts:
